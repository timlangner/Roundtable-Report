...
Build or obtain `EldenRing_StatsShare.dll`, then pick the loader you already use.

### Elden Ring Mod Loader (`Game\mods`)

Copy both files into `steamapps\common\ELDEN RING\Game\mods` (same folder as other native DLLs such as UnlockTheFps):
...
Leave `webhook_url` empty to keep local tracking only.
